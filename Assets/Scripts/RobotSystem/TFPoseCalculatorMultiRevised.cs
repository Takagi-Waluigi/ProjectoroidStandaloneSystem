...
public class TFPoseCalculatorRevised : MonoBehaviour
{
    ...
    [SerializeField] bool readNamespaceFromTransporter = false;
    ...
    void Start()
    {
        if(readNamespaceFromTransporter)
        {
            rosObject = GameObject.Find("ROSConnectionPrefab(Clone)");
            ValueTransport valueTransport = rosObject.GetComponent<ValueTransport>();
            Debug.Log("Namespace:" + valueTransport.rosNamespace);

            rosNamespace = valueTransport.rosNamespace;
        }
